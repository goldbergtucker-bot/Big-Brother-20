...
(function () {
  ...
  function planBackdoor(state, hoh, nominees) {
    ...
    // A backdoor is a deliberate HOH plan, not something that should only
    // be discovered after nominations. Keep it selective, but make the
    // planner reachable often enough that a full season can naturally
    // contain several intentional attempts.
    const threshold = 48 - (hohStrategic - 50) * 0.08;
    const baseChance = 0.30 + Math.max(0, hohStrategic - 50) / 170;
    const use = best.score >= threshold && Math.random() < Math.min(0.48, baseChance);
